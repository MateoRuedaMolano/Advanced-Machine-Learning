...
from utils.utils import get_optimizer, batch_to_var, batch_to_var_nlb, make_dir, check_parallel
...
def runIter(args, encoder, decoder, x, y_mask, y_mask_auxiliar,sw_mask,
            crits, optims, mode='train', loss = None, prev_hidden_temporal_list=None, prev_mask = None, last_frame=False):
    """
    ...
    for t in range(0, T):
        ...
        out_mask_auxiliar,out_mask, hidden = decoder(feats, mask_lstm, hidden_spatial, hidden_temporal)
        hidden_tmp = []
        ...
        out_mask_auxiliar = out_mask_auxiliar.view(out_mask_auxiliar.size(0),-1)
        out_mask = out_mask.view(out_mask.size(0), -1)

        # get predictions in list to concat later
        out_masks_auxiliar.append(out_mask_auxiliar)
    ...
    out_masks_auxiliar = torch.cat(out_masks_auxiliar,1).view(out_mask_auxiliar.size(0),len(out_masks_auxiliar), -1)
    ...
    if args.use_gpu:
        ...
        out_masks_auxiliar = out_masks_auxiliar.contiguous()
    ...
    loss_mask_iou_auxiliar = mask_siou(y_mask_auxiliar.view(-1,y_mask_auxiliar.size()[-1]),out_masks_auxiliar.view(-1,out_masks_auxiliar.size()[-1]), sw_mask.view(-1,1))
    loss_mask_iou_auxiliar = torch.mean(loss_mask_iou_auxiliar)

    # total loss is the weighted sum of all terms
    if loss is None:
        #loss = args.iou_weight * loss_mask_iou
        loss = args.iou_weight * (loss_mask_iou + loss_mask_iou_auxiliar)
    else:
        #loss += args.iou_weight * loss_mask_iou
        loss += args.iou_weight * (loss_mask_iou + loss_mask_iou_auxiliar)
    ...
    for e in range(args.max_epoch):
        ...
        for split in ['train']:
            if args.dataset == 'davis2017' or args.dataset == 'youtube':
                for batch_idx, (inputs, targets,targets_auxiliar,seq_name,starting_frame) in enumerate(loaders[split]):
                    # send batch to GPU
                    ...
                    for ii in range(max_ii):
                        ...
                        x, y_mask, y_mask_auxiliar, sw_mask = batch_to_var_nlb(args, inputs[ii], targets[ii],targets_auxiliar[ii])
                        ...
                        loss, losses, outs, hidden_temporal_list = runIter(args, encoder, decoder, x, y_mask, y_mask_auxiliar, sw_mask,
                                                                                            crits, optims, split,
            ...
            if split == 'train' and args.smooth_curves:
