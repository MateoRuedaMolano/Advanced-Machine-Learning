...
class MyDataset(data.Dataset):
        ...
        if self.video_mode:
            if self.split == 'train' or self.split == 'val' or self.split == 'trainval':
                ...
                for ii in range(max_ii):
                    ...
                    #annot_auxiliar= imresize(annot,(30,54)) #DAVIS
                    annot_auxiliar= imresize(annot,(32,56)) #YOUTUBE
                    ...
                    target,target_auxiliar = self.sequence_from_masks(seq_name,annot,annot_auxiliar)
                    ...
                    targets_auxiliar.append(target_auxiliar)
                                    
                #return imgs, targets, seq_name, starting_frame
                return imgs, targets, targets_auxiliar,seq_name, starting_frame
    ...
    def sequence_from_masks(self, seq_name, annot,annot_auxiliar):
        """
        ...
        if self.dataset == 'youtube':
            ...
            instance_ids = np.unique(annot)[1:] #how many instances has the gt
        ...
        h = annot.shape[0]
        w = annot.shape[1]
        h_auxiliar = annot_auxiliar.shape[0]
        w_auxiliar = annot_auxiliar.shape[1]
        ...
        gt_seg_auxiliar = np.zeros((num_instances, h_auxiliar*w_auxiliar)) #Useful for NLBx3 or NLBx4 mIoU
        ...
        for i in range(total_num_instances):
            ...
            sample_weights_mask[id_instance-1] = 1 #1 where the mask is not null

            aux_mask_auxiliar = np.zeros((h_auxiliar, w_auxiliar))  #Useful for NLBx3 or NLBx4 mIoU
            gt_seg_auxiliar[id_instance-1,:] = np.reshape(aux_mask_auxiliar,h_auxiliar*w_auxiliar) #Useful for NLBx3 or NLBx4 mIoU

        gt_seg = gt_seg[:][:self.max_seq_len]
        sample_weights_mask = sample_weights_mask[:][:self.max_seq_len]

        gt_seg_auxiliar = gt_seg_auxiliar[:][:self.max_seq_len]

        targets = np.concatenate((gt_seg,sample_weights_mask),axis=1)
        targets_auxiliar = np.concatenate((gt_seg_auxiliar,sample_weights_mask),axis=1)

        #return targets
        return targets, targets_auxiliar
