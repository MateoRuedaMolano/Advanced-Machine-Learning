...
def batch_to_var_nlb(args, inputs, targets,targets_auxiliar):
    """
    Turns the output of DataLoader into data and ground truth to be fed
    during training
    """
    x = Variable(inputs,requires_grad=False)
    y_mask = Variable(targets[:,:,:-1].float(),requires_grad=False)
    y_mask_auxiliar = Variable(targets_auxiliar[:,:,:-1].float(),requires_grad=False)
    sw_mask = Variable(targets[:,:,-1],requires_grad=False)

    if args.use_gpu:
        return x.cuda(), y_mask.cuda(), y_mask_auxiliar.cuda(),sw_mask.cuda()
    else:
        return x, y_mask, y_mask_auxiliar, sw_mask
