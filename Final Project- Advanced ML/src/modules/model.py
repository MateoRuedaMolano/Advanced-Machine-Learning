...
from .clstm import ConvLSTMCell, ConvLSTMCellMask, ConvGRUMask, ConvGRUCellMask
import argparse
...
from .NLN import NONLocalBlock2D
...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self,args):
        ...
        self.nlb5 = NONLocalBlock2D(2048,bn_layer=False)
        self.nlb4 = NONLocalBlock2D(1024,bn_layer=False)
        self.nlb3 = NONLocalBlock2D(512, bn_layer=False)
        self.nlb2 = NONLocalBlock2D(256, bn_layer=False)
    ...
    def forward(self,x,semseg=False, raw = False,nlb=False):
...
class RSISMask(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(len(skip_dims_out)):
            ...
            if args.RNN == 'GRU':
                clstm_i = ConvGRUCellMask(args, clstm_in_dim, skip_dims_out[i],self.kernel_size, padding = padding)
            else:
        ...
        self.conv_out_auxiliar = nn.Conv2d(skip_dims_out[0], 1,self.kernel_size, padding = padding)
    ...
    def forward(self, skip_feats, prev_mask, prev_state_spatial, prev_hidden_temporal):     
        ...
        for i in range(len(skip_feats)+1):
            ...
            if i < len(skip_feats):
                if i ==2:
                    out_mask_auxiliar = self.conv_out_auxiliar(clstm_in)
        ...
        return out_mask_auxiliar,out_mask, hidden_list
